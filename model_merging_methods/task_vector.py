...
class TaskVector:
    def __init__(self, pretrained_model: nn.Module = None, finetuned_model: nn.Module = None, exclude_param_names_regex: list = None, task_vector_param_dict: dict = None):
        ...
        if task_vector_param_dict is not None:
            ...
            # else:
                # param_names_to_merge = get_param_names_to_merge(input_param_names=list(pretrained_param_dict.keys()), exclude_param_names_regex=exclude_param_names_regex, locknums=locknums)
                # pdb.set_trace()
            with torch.no_grad():
                for param_name in param_names_to_merge:
                    ...
                    # except:
                    #     try:
                    #         self.task_vector_param_dict[param_name] = finetuned_param_dict['language_model.'+param_name] - pretrained_param_dict[param_name]
                    #     except:
                    #         # print(param_name)
                    #         # pdb.set_trace()
                    #         if finetuned_param_dict['language_model.'+param_name].shape[0] > pretrained_param_dict[param_name].shape[0]:
                    #             n = pretrained_param_dict[param_name].shape[0]
                    #             self.task_vector_param_dict[param_name] = finetuned_param_dict['language_model.'+param_name][:n] - pretrained_param_dict[param_name]
    ...
    def __add__(self, other):
        ...
        with torch.no_grad():
            for param_name in self.task_vector_param_dict:
                ...
                if param_name in other.task_vector_param_dict:
    ...
    def combine_with_param_dict(self, pretrained_param_dict,scaling_coefficient):
        with torch.no_grad():
            merged_params = {}
            for param_name in self.task_vector_param_dict:
                merged_params[param_name] = pretrained_param_dict[param_name] + scaling_coefficient * self.task_vector_param_dict[param_name]
