    ...
    if disentangle:
        ...
        for i in range(nsamples):
            ...
            attention_mask = torch.cat(
                (trainenc_prompt.attention_mask, trainenc_response.attention_mask[:, 1:]), dim=1
            )
            tar = inp.clone()
            trainenc_prompt_len = trainenc_prompt.input_ids.shape[1]
            tar[:, :trainenc_prompt_len] = -100
            trainloader.append((inp, tar, attention_mask))
    else:
        # Encode datasets
...
def get_code(nsamples, seed, seqlen, tokenizer, disentangle=False, dataset="math"):
    # data_files = {"train" : "data/143k-Tested-Python-Alpaca-Vezora-clean.json"}
    data_files = {"train" : "data/humaneval.parquet"}
    ...
    if disentangle:
        ...
        for i in range(nsamples):
            ...
            trainenc_response = tokenizer(
                traindata_sampled["canonical_solution"][i], return_tensors="pt"
            )
            inp = torch.cat(
                (trainenc_prompt.input_ids, trainenc_response.input_ids[:, 1:]), dim=1
...
def get_math(nsamples, seed, seqlen, tokenizer, disentangle=False, dataset="math"):
    # data_files = {"train" : "./locate/data/MathInstruct.json"}
    ...
    if disentangle:
        ...
        for i in range(nsamples):
            trainenc_prompt = tokenizer(
                traindata_sampled["question"][i], return_tensors="pt"
            )
            trainenc_response = tokenizer(
                traindata_sampled["answer"][i], return_tensors="pt"
            )
            inp = torch.cat(
                (trainenc_prompt.input_ids, trainenc_response.input_ids[:, 1:]), dim=1
            )  # to remove the first token of the response ('1')
...
def get_loaders(
    ...
    if name == "math":
        return get_math(
            nsamples, seed, seqlen, tokenizer, disentangle=disentangle
        )
    if name == "code":
        return get_code(
            nsamples, seed, seqlen, tokenizer, disentangle=disentangle
